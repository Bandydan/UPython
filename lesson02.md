# Урок 2. Булева алгебра. Керуючі конструкции.

## Булеві змінні

Змінна типу 'bool' може містити тільки два значення: **True** або **False**, при чому це не рядки, не числа, а саме поняття брехня і істина. Для роботи з булевими змінними існує булева алгебра.
...
* and - та 
* or - або
* xor - виключаюче або (підключається окремо)
* not - НЕ not означає НЕ, і, будучи поставленим перед типом bool, міняє його значення на зворотне, тобто **'not True'** стає **'False'**, **'not False'** стає **'True'** Далі приведена таблиця, що демонструє роботу операторів :
...
В python до версії 3.10 існвав тільки один умновний операторʼ `if -> elif -> else`. Починаючи з версії мови 3.10 був доданий умовний оператор `match/case`. Поки що ця версія мови не є офіційно прийнятою усюди, але якщо вам цікаво, як це працює, в кінці уроку є посилання на відповідну статтю. 
...
Останні чотири оператора потребують додаткового пояснення.

Оператор `is` перевіряє, чи є обидва операнда одним і тим самим об'єктом у пам'яті, тоді як просте порівняння на рівність `==` перевіряє тільки на відповідність вмісту двох операндів, але не перевіряє, чи є вони тим самим об'єктом.
...
Тернарні оператори часто застосовуються, коли варіант всього два, так як такий запис короче аналогічної дії з if. Однак, на жаль, можна зустріти зв'язок if-ів й тернарних операторів, який є важким для розуміння. Інакше кажучи, це не потрібна річ у мові програмуування, і вона може стати причиною погіршення розуміння коду.
...
Python вважає, що якщо об'єкт не пустий, він `True`, а не `False`. `False` - це власне сам `False`, 0 і пуста строка. Таким чином, якщо нам необхідно порівняти строку з пустою строкою, або число з нулем, а також дізнатися, чи є пустим той чи інший об’єкт, замість того, щоб написати умови в стилі старих мов:
...
В нашому випадку test = `True`, `and` його пропускає і повертає нам останнє істинне значення, з яким працював, тобто выраз «Test is True' or 'Test is False». Or повертає перший зустрічний True вираз, йому більше нічого не цікаво. Якщо замінити рядки наближеним змістом і використати псевдокод, отримаємо щось потипу:
...
2. Перевірити, чи є числоне парним, ділиться чи на три і на п'ять одночасно, але так, щоб не ділитися на 10.
...
[Приоритети операций в Python](https://docs.python.org/3/reference/expressions.html#operator-precedence)
